#!/bin/bash
# Setup/maintenance script, not usually a live-show button: reads OBS's
# current scene list and rewrites config.yml's main playlist from it.
...
cd "$(dirname "$0")" || exit 1
exec ruby bin/run.rb populate main
